/**
 * Gemini 3 Flash Preview + Web Grounding テスト
 * 
 * gemini-3-flash-preview で Web Grounding (Google 検索) が動作するか確認します。
 * 今日のニュースを取得して、最新情報が返ってくるかテストします。
 * 
 * 実行方法:
 *   npx tsx test/gemini3-flash-grounding-test.ts
 */

import { GoogleGenAI } from '@google/genai';

const PROJECT_ID = process.env.GOOGLE_CLOUD_PROJECT || 'shinise-master-2026-e4a9e';
const LOCATION = 'global';  // Gemini 3 は global で利用可能
const MODEL_ID = 'gemini-3-flash-preview';

async function testGemini3FlashWithGrounding() {
  console.log('🚀 Gemini 3 Flash Preview + Web Grounding テスト');
  ...
  try {
    ...
    console.log('\n🎉 Gemini 3 Flash + Web Grounding テスト成功！');
